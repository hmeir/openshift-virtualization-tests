...
def assert_updated_hco_default_cpu_model(hco_resource, expected_cpu_model):
    hco_cpu_model = (
        hco_resource.instance
        .to_dict()["spec"]
        .get(VIRTUALIZATION_KEY, {})
        .get(VIRTUAL_MACHINE_OPTIONS_KEY, {})
        .get(HCO_DEFAULT_CPU_MODEL_KEY)
    )
    ...
    with ResourceEditorValidateHCOReconcile(
        ...
        patches={
            hyperconverged_resource_scope_function: {
                "spec": {
                    VIRTUALIZATION_KEY: {
                        VIRTUAL_MACHINE_OPTIONS_KEY: {HCO_DEFAULT_CPU_MODEL_KEY: cluster_common_node_cpu}
                    },
                },
            }
        },
...
def test_default_value_for_cpu_model(
    ...
    """
    assert HCO_DEFAULT_CPU_MODEL_KEY not in hco_spec_scope_module.get(VIRTUALIZATION_KEY, {}).get(
